...
def tutor_dashboard():
    ...
    with tabs[2]:
        ...
        if st.button("End Meeting"): db.end_meeting(); st.info("Meeting ended.")
        logs = db.get_all_activities()
        st.subheader("Student Activity Logs")
        for log in logs.values(): st.write(f"🕒 {log['timestamp']} — {log['user']}: {log['action']}")
...
def student_dashboard(username):
    ...
    with tabs[2]:
        ...
        for id, doubt in db.get_doubts().items():
            st.markdown(f"**{doubt['user']} asked:** {doubt['question']}")
            for ans in doubt["answers"]: st.write(f"💬 {ans}")
            reply = st.text_input(f"Reply to {id}", key=id)
            if st.button("Post Answer", key=f"btn_{id}"): db.post_answer(id, reply); st.success("Answer posted!")
    with tabs[3]:
